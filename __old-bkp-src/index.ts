...
export async function checkExists(
  ...
  if (
    !sha ||
    (sha && sha.length !== 64) ||
    (sha && !/^[\dA-Fa-f]{64,}$/.test(sha))
  ) {
    return {
      ok: false,
      error: {
        message:
          'Invalid SHA-256 hash, must be 64 hex characters long, or 66 if 0x-prefixed',
        httpStatus: 400,
      },
    };
  }
  ...
  if (/content|data/i.test(type)) {
    ...
    return {
      ...
      headers: opts.headers
        ? {
            ...opts.headers,
            // if the content is gzip encoded, set the content-encoding header so it could be received properly by the caller
...
export async function estimateDataCost(
  ...
  try {
    ...
    const totalGasWei = opts.gasPrice
      ? opts.gasPrice * 1e9
      : opts.baseFee + opts.priorityFee;
