...
export async function upstreamFetcher(
  ...
  if (opts.resolve) {
    opts = (await resolveAddressPatches(opts)) as typeof opts;
  }

  const { baseURL } = opts;

  // @ts-expect-error - we don't want to pass these to the fetch
  delete opts.resolve;
  // @ts-expect-error - we don't want to pass these to the fetch
  delete opts.baseURL;
  ...
  } as OkShape<Uint8Array | Record<string, any>>;
...
export function normalizeResult(result: any, options?: any): EthscriptionBase {
  ...
  return withRes.length > 0
    ? (withRes as typeof withRes)
    : (onlyRes as typeof onlyRes);
}
...
export function numfmt(x: string, delim = ','): string {
  if (!x) return '';
...
export function isHex(val: string, prefixed = false): boolean {
  return Boolean(
    val &&
      /^[\dA-Fa-f]+$/.test(val) &&
      val.length % 2 === 0 &&
